...
struct QueueItem
{
    ...
    Str sectionsWarning;
    /// «Скачать ещё раз»: существующий файл заменяется, а не пропускается.
    /// Старый файл живёт до конца загрузки — yt-dlp качает в .part и только
    /// при успехе переименовывает поверх.
...
public:
    ...
    struct Options
    {
        ...
        int durationHint = 0;
        /// Существующий итоговый файл перекачивается с заменой.
        bool forceOverwrite = false;
    };
    ...
    /// Имя файла из заголовка записи — зеркало sanitize_filename yt-dlp
    /// (обычный режим, не restricted). maxBytes > 0 повторяет усечение
    /// шаблона %(title).NB: сырые байты режутся ДО санитизации.
    static Str ytFileName (const Str& title, int maxBytes = 0);

    /// Предсказание итоговых файлов по папке назначения — источник правды
    /// для окна «этот файл уже скачан». Просит список заявок
    /// {"files":[{…}]} и отвечает {"results":[{…}]} (см. CAPI). Статический
